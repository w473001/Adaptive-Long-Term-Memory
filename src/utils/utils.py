...
def get_task_epoch_matrix(n_tasks: int, T: int, super_pool_size: int, epoch_pool_size: int, epoch_block_pool_size: int, n_epochs: int,
    ...
    if random:
        ...
        epoch_pools = [{t: np.random.choice(a=epoch_block_pools[i], size=epoch_pool_size, replace=False) for t in range(n_tasks)} for i in range(n_epochs)]
...
def projection(w, beta):
    """
    Projects the point w onto the simplex with lower box constraints defined by beta. Projection is wrt relative
    entropy. Requires beta to be non-negative in all elements and to sum less than 1.

    Computes the projection O(n) time.

    :param w:
    :param beta:
    :return: projection(w; C_beta)
    """
    if all([wi >= b for wi, b in zip(w, beta)]):
        return w
    weights = w.copy()
    ratios = weights / beta
    w_set = np.array([i for i in range(len(weights))])
    c_sum = 0
    c_beta_sum = 0
    threshold = None
    while len(w_set) > 0:
        threshold = np.median(ratios[w_set])

        l_set = w_set[ratios[w_set] < threshold]
        l_beta_sum = beta[l_set].sum()
        l_sum = weights[l_set].sum()

        m_set = w_set[ratios[w_set] == threshold]
        m_beta_sum = beta[m_set].sum()
        m_sum = weights[m_set].sum()

        h_set = w_set[ratios[w_set] > threshold]

        gamma = (1 - (c_beta_sum + l_beta_sum)) / (1 - (c_sum + l_sum))

        if (threshold * gamma) < 1:
            c_sum += l_sum + m_sum
            c_beta_sum += l_beta_sum + m_beta_sum
            if len(h_set) == 0:
                threshold = np.min(ratios[ratios > threshold])
            w_set = h_set
        else:
            w_set = l_set

    gamma = (1 - c_beta_sum) / (1 - c_sum)
    weights[ratios < threshold] = beta[ratios < threshold]
    weights[ratios >= threshold] *= gamma
    assert(np.isclose(weights.sum(), 1.0))
    assert(all([w >= b for w, b in zip(weights, beta)]))
    return weights
