...
class AdaptLTM(Algo):
    ...
    def __init__(self, n_tasks, n_experts, alpha, rho, **kwargs):
        ...
        self.beta_td[:, :, 1] = (1 - self.alpha) / 2
        self.beta_temp = self.beta.copy()
        self.beta_td_temp = self.beta_td.copy()

        self.beta_2q_weights_ = np.array([[1.0*(2**q) for q in range(self.beta.shape[-1])] for _ in range(n_tasks)]).reshape((n_tasks, self.beta.shape[-1]))
    ...
    def predict(self, task_number, expert_predictions):
        ...
        self.sigma = (self.beta[task_number, :, :] * self.beta_2q_weights_[task_number, :]).sum(axis=1)
        self.sigma_td = (self.beta_td[task_number, :, :] * self.beta_2q_weights_[task_number, :]).sum(axis=1)
    ...
    def switching_update(self, task_number):
        ...
        self.beta_temp[task_number, :, :] = self.beta_temp[task_number, :, :] * self.psi.reshape((self.psi.shape[0], -1))

        # When beta changes, reset the weight
        beta_2q_weights_ = self.beta_2q_weights_[task_number, :].copy()
        beta_2q_weights_ = beta_2q_weights_ - self.alpha
        if qt == 0:
            ...
            beta_2q_weights_[0] = 2 ** 0
            beta_2q_weights_[1] = 2 ** 1
        else:
            ...
            beta_2q_weights_[0] = 2 ** 0
            beta_2q_weights_[qt] = 2 ** qt
            beta_2q_weights_[qt+1] = 2 ** (qt+1)

        self.beta_2q_weights_[task_number, :] = beta_2q_weights_.copy()
    ...
    def log_dump(self):
        ...
        logger.info(f"t = {self.t}, phi={self.phi.sum()}")
